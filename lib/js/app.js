	...
	Canvas.prototype.init = function() {
		...
		*************/
		...
		while (active.length > 0) {
			...
			// Get a random index
			var rand = getRandomNumInt(1,active.length);
			...
			for (var n = 0; n < this.constant; n++) {
				...
				var sample = {
					x: getRandomNumInt(0,this.canvas.width),
					y: getRandomNumInt(0,this.canvas.height)
				};
				...
				var newX = pos.x + Math.sin(angle)*magnitude;
				var newY = pos.y - Math.cos(angle)*magnitude;

				var circlePoint = new Circle (newX,newY, 4, '#000', this);
				circlePoint.draw();
...
/*
				...
				var angle = getRandomNumInt(0,2*Math.PI);
				...
				magnitude = getRandomNumInt(testRadius,testRadius*2);
